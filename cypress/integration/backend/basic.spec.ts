    ...
    () => {
      ...
      it("Send undefined values in request data", done => {
        ...
        adapter.request("common/sendArr", data).then((res: any) => {
          ...
          expect(res.table1[1][0]).to.be.equal(data.table1[0].col1);
          expect(res.table1[1][1]).to.be.equal(data.table1[0].col2);
          expect(res.table1[1][2]).to.be.equal(data.table1[0].col3);
          expect(res.table1[2][0]).to.be.equal(data.table1[0].col1);
          expect(res.table1[2][1]).to.be.equal(data.table1[0].col2);
          expect(res.table1[2][2]).to.be.equal(data.table1[0].col3);
          done();
        });
      });
      ...
      it("Send null values in request data", done => {
        ...
        const data: any = {
          table1: [
            { col1: "first col value", col2: null, col3: null },
            { col1: "second row vlaue", col2: 0, col3: "value" },
            { col1: "second row vlaue", col2: null, col3: null }
          ]
        };
        adapter.request("common/sendArr", data).then((res: any) => {
          testFinish();
      ...
      it("sasjs instantiated with no properties", done => {
        ...
        expect(sasConfig.serverUrl).to.be.equal(defaultConfig.serverUrl),
          expect(sasConfig.port).to.be.equal(defaultConfig.port);
      ...
      it("sasjs instantiated with all properties", done => {
        ...
        expect(sasConfig.serverUrl).to.be.equal(config.serverUrl),
          expect(sasConfig.port).to.be.equal(config.port);
